...
import { useCreateCollection, useImportCreateCollection } from "./endpoint-hooks";
...
export function CreateCollectionForm(props: CreateCollectionFormProps): React.ReactElement {
  ...
  const { importCreateCollection } = useImportCreateCollection();
  ...
  const [loadingImportProgress, setLoadingImportProgress] = useState<number>(null);
  const [errors, setErrors] = useState<CreateCollectionValidationFormModel>({ name: "", owners: "" });
  ...
  const handleSubmit = () => {
    const formValid = validateCreateCollectionForm({ name, owners }, setErrors);
  ...
  const importGamesAndCreateCollection = (files) => {
    ...
    reader.onload = (event) => {
      ...
      if (formValid) {
        importCreateCollection({
          collectionName: name,
          collectionJson: parsedJson,
          collectionOwners: owners,
          onCollectionWithImportCreated: onCollectionCreated,
          setIsLoading: setIsLoadingImport,
          setLoadingProgress: setLoadingImportProgress,
        });
      }
    };
  };
                ...
                <DropzoneDialog
                  dialogTitle={(
                    ...
                    <Grid container item xs={11} spacing={1}>
                      ...
                      {isLoadingImport && (
                        <LinearProgress value={loadingImportProgress * 100} />
                      )}
                    </Grid>
                  )}
                  filesLimit={1}
                  ...
                  onSave={importGamesAndCreateCollection}
                  showPreviews
