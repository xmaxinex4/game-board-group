...
echo REDIS_CLUSTER_HOST=$(aws secretsmanager get-secret-value --region us-west-2 --secret-id RedisEndpoint --query SecretString --output text | jq -r .RedisEndpoint) >> .env
echo REDIS_CLUSTER_PORT=$(aws secretsmanager get-secret-value --region us-west-2 --secret-id RedisPort --query SecretString --output text | jq -r .RedisPort) >> .env
